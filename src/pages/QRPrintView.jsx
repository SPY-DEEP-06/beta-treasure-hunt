  ...
  return (
    <div className="bg-white min-h-screen text-black p-8">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-8 print:grid-cols-2">
        {initialClues.map(loc => (
          <div key={loc.clueId} className="border-4 border-black p-6 flex flex-col items-center text-center break-inside-avoid shadow-sm rounded-xl">
            <h2 className="text-2xl font-black mb-2 uppercase">Beta Hunt</h2>
            <p className="text-sm font-bold mb-4 bg-black text-white px-3 py-1 rounded w-full">Location {loc.clueId}</p>
            <QRCode
              value={JSON.stringify({ locationId: loc.clueId })}
              size={180}
              level="H"
            />
            <p className="mt-4 text-xs font-mono">{loc.title}</p>
          </div>
        ))}
      </div>
    </div>
